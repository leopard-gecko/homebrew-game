...
ここで配布しているパッケージは、Rogueのオリジナル系統の最終形である**バージョン5.4**と、クローン系統の最終形である**クローン・バージョン3**（以下、**Clone III**と表記）をもとに、それぞれのバージョンに対してUTF-8による日本語・英語表示切り替え、日本語や絵文字の入力機能、カラー表示、macOS向けのビルド修正などを加えたものです。Homebrewの公式リポジトリに英語版Rogueのパッケージがすでに存在しているため、こちらのパッケージは `jrogue`  、`jrogue-clone`という名前にしています。
...
5.4版の実行コマンドは `jrogue` です。Homebrew版では遊びやすい初期設定でビルドされます。オプションの初期設定は、カラー有効、`passgo` 有効、識別の巻き物1種類、プレイ中のカーソル非表示です。
...
Clone III版の実行コマンドは `jrogue-clone` になります。初期設定は、カラー有効、`passgo` 無効、`jump` 無効、墓標表示あり、終了確認あり、プレイ中のカーソル非表示です。
