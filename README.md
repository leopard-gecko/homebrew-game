...
ソースから何も指定せずにビルドした場合は、実行ファイル名が `rogue` になり、各ファイル名も `rogue.save`、`rogue.scr`、`rogue.lck` になります。`./configure --program=名前` または `make PROGRAM=名前` を使うと、実行ファイル名に連動して標準のセーブファイル、スコアファイル、ロックファイル名も変わります。
