...
1つの実行ファイルで表示言語を切り替えられます。日本語ロケールでは日本語、それ以外では英語になりますが、ゲーム中での切り替えも随時可能です。Homebrew版では、普通に遊びやすい `jrogue`、Rogue本来に近い初期設定の `jrogue-original`、ウィザードモードを有効にした `jrogue-wizard` を別々のFormulaとして用意しています。
...
通常版の `jrogue` は、現代のターミナルで遊びやすい初期設定でビルドされます。初期状態はカラー有効、`passgo` 有効、識別の巻き物1種類、プレイ中のカーソル非表示です。普通に遊ぶ場合はこちらがおすすめです。

Rogue本来に近い初期設定で遊びたい場合は、次のFormulaをインストールします。初期状態はカラー無効、`passgo` 無効、識別の巻き物5種類、プレイ中のカーソル表示ありです。

```sh
brew install jrogue-original
```

ウィザードモードありで遊びたい場合は、次のFormulaをインストールします。`jrogue-wizard` は通常版と同じ便利設定に加えて、ウィザードモードを有効にしてビルドされます。

```sh
brew install jrogue-wizard
```

遊び方やライセンスなどの各種ドキュメントは、Homebrewの通常のドキュメントディレクトリにインストールされます。Apple Silicon環境で通常版 `jrogue` をインストールした場合は、おおむね次の場所になります。

```sh
/opt/homebrew/share/doc/jrogue/
```

`jrogue-original` と `jrogue-wizard` では、それぞれ次の場所になります。

```sh
/opt/homebrew/share/doc/jrogue-original/
/opt/homebrew/share/doc/jrogue-wizard/
```

Intel環境では、Homebrewのインストール先に応じて `/usr/local/share/doc/` 以下に同じ名前でインストールされます。
...
manページは、実行ファイル名に合わせて `jrogue.6`、`jrogue-original.6`、`jrogue-wizard.6` としてインストールされます。
...
| `jrogue-original` | `jrogue-original` | カラー無効、`passgo` 無効、識別の巻き物5種類、プレイ中のカーソル表示あり |
| `jrogue-wizard` | `jrogue-wizard` | `jrogue` と同じ便利設定に加えて、ウィザードモードあり |

表示言語はどのFormulaでも固定されません。`ROGUE_LANG` を指定していない場合はロケールに従い、日本語ロケールでは日本語、それ以外では英語になります。

以前の説明にあった `--with-modern-settings`、`--with-japanese`、`--with-english`、`--with-wizardmode` は使いません。Homebrewで種類を選びたい場合は、上記のFormulaを選んでインストールしてください。
...
`jrogue-original` や `jrogue-wizard` を使う場合は、上記の `jrogue` をそれぞれの実行コマンド名に読み替えてください。
...
- `color`：カラーで表示します。`jrogue` と `jrogue-wizard` では有効、`jrogue-original` では無効が初期値です。
- `cursor`：プレイ中のカーソルを表示します。`jrogue-original` では有効、`jrogue` と `jrogue-wizard` では `@` を見やすくするため無効が初期値です。
- `idscrl`：識別の巻き物を1種類にまとめます。ゲーム開始後は変更できません。`jrogue` と `jrogue-wizard` では有効、`jrogue-original` では無効が初期値です。
...
Homebrew版では、`jrogue`、`jrogue-original`、`jrogue-wizard` のどれを使っても、標準のセーブファイル、スコアファイル、ロックファイルは共通になります。

| Formula | セーブファイル | スコアファイル | ロックファイル |
| ------- | -------------- | -------------- | -------------- |
| `jrogue` | `jrogue.save` | `jrogue.scr` | `jrogue.lck` |
| `jrogue-original` | `jrogue.save` | `jrogue.scr` | `jrogue.lck` |
| `jrogue-wizard` | `jrogue.save` | `jrogue.scr` | `jrogue.lck` |

Formula側では、実行ファイル名をそれぞれ `jrogue`、`jrogue-original`、`jrogue-wizard` に分けつつ、`./configure --save-file=jrogue.save --score-file=jrogue.scr --lock-file=jrogue.lck` を指定して、保存先の標準ファイル名だけを共通にしています。
...
以下のように `.zshrc` に記述すると、日本語表示、カラー表示、カーソル無効、識別の巻き物が１種類、簡潔表示、通路を止まらずに進む設定で起動し、ゲーム終了後にもう一度遊ぶかを尋ねるようになります。
