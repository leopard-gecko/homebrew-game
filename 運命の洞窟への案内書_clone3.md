**【運命の<ruby>洞窟<rp>（</rp><rt>ダンジョン</rt><rp>）</rp></ruby>】への案内書 Clone III版**
================
...
f        | "fight"。コマンドの後に方向を指定すると、その方向にいる怪物と戦う。
F        | より徹底して戦うためのfightコマンド。方向を指定して使う。
m        | "move onto without picking up"。物を拾わずに何かの上に移動する。指定した方向に一コマ進むが、そこに拾えるものがあっても拾わない。
z        | "zap"(素早く振る)の接頭辞コマンド。魔法の杖を指定した方向に向けて発射する。方向性のない杖であっても、使う時にはどこかの方向を指す必要がある。
...
Q        | "quit"。終了する。ゲームを離れる。。
S       | "save game"。現在のゲームをファイルに保存する。デフォルトのセーブファイルを使うかを尋ねる。警告：ローグはセーブのコピーは起動させない上に、セーブされたゲームを再開すると直ちにセーブファイルを削除する。これは、危険なところでゲームをセーブして死んでしまったらゲームをやり直すことをさせないようにするためである。<br>セーブファイルをデフォルトで保存した場合は、ゲームを再開するためには以下のようにする。<br>$ jrogue-clone rogue-clone.save<br>ファイル名を指定して保存した場合は、以下のようにローグの引数としてファイル名を指定する。<br>$ jrogue-clone *save\_file*
...
洞窟の部屋は、壁と床と扉によって区切られている。部屋に入ると、部屋の中の地形や見えているものが画面に描かれる。通路は部屋と部屋をつなぎ、隠し扉やわなが潜んでいることもある。
...
無し               |  0  
革のよろい            |  2  
<ruby>鉄環<rp>（</rp><rt>てっかん</rt><rp>）</rp></ruby>のよろい |  3  
...
魔法の杖は、木の種類で識別される。たいていは遠く離れたものに何かをしたい時に使われるものなので、効果を及したいものへの方向を示す必要がある。しかし、方向には関係がない杖もある。杖には魔法のチャージがあり、その量はランダムであり、使い切った杖はただの木になってしまう。

魔法の杖を使うためのコマンドは、「z」(zap：素早く振る)である。
...
環境変数「ROGUEOPTS」は、さまざまなオプションの初期値をカンマで区切ってリストにした文字列である。Boolean変数はその名前をリストに入れることにより有効にすることができ、名前の前に"no"を加えると無効にすることができる。したがって「**jump**」(移動中の表示を行わない)を有効に、「**passgo**」(通路の角で止まらずに進む)を有効、「**name**」（プレイヤー名）を「ブルー・ミーニー」になるように環境変数を設定するには、次のコマンドを使用する。

$ export ROGUEOPTS="jump,passgo,name=ブルー・ミーニー"

表示する言語については、環境変数「ROGUE_LANG」を使って起動時に指定することもできる。

$ ROGUE_LANG=ja jrogue-clone

$ ROGUE_LANG=en jrogue-clone

また、「ROGUEOPTS」に「language=ja」または「language=en」を含めて指定することもできる。

$ export ROGUEOPTS="passgo,color,language=ja"

### **8.2.  オプションのリスト**

ここではオプションの一覧と、それぞれのオプションの説明を示す。それぞれのデフォルトの値は角括弧で囲まれている。文字列オプションの場合、長すぎる入力は無視または切り詰められる。

**jump**［*nojump*］（移動中の表示を行わない）

> このオプションが設定されている場合、移動が終わるまで表示されない。これによりCPUと表示の時間がかなり節約される。

**passgo**［*nopassgo*］（通路の角で止まらずに進む）

> 通路の曲がり角に従って進む。通路を通って石や壁にぶつかると、ローグは右か左のどちらかに曲がれるかどうかを判別する。片方向だけに曲がれる場合にはその方向に曲がる。どちらにも曲がれるかどちらにも曲がれない場合は立ち止まる。

**skull / tombstone**［*skull*］（死んだときに墓標を表示）

> プレイヤーが死んでしまったとき、最後に墓標を表示する。望むなら *noskull* または *notombstone* で無効にできる。

**askquit**［*askquit*］（終了時に確認する）

> 終了時に確認を行う。*noaskquit*を指定すると確認なしに終了する。

**name**［*アカウント名*］（プレイヤー名）

> プレイヤーの名前。得点者リストのトップに入ったときに使われる。
...
> 各種のキャラクターがカラーで表示される。カラーを無効にしたい場合は *nocolor* を指定する。

**cursor**［*nocursor*］（プレイ中のカーソルを表示）

> プレイ中にもカーソルを表示する。無効にすると通常の移動中はカーソルを隠し、入力が必要な場面ではカーソルを表示する。

**language**［*日本語(J) / 英語(E)*］（表示する言語）

> 表示する言語を切り替える。Jで日本語、Eで英語になる。オプション設定画面で切り替えた場合、以後のメッセージや表示名は選択した言語で再生成される。起動時の言語は「ROGUE_LANG=ja」または「ROGUE_LANG=en」で指定することもできる。

**file**［*rogue-clone.save*］（セーブファイル名）

> ゲームをセーブするためのデフォルトのファイル名。ファイル名は「~」で始めるとホームディレクトリとして扱われる。
...
*日本語訳、Clone III版向け修正、日英・カラー対応版およびmacOS用の修正・追加：leopardgecko*
